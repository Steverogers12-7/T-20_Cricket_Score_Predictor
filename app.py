...
temp_overs = overs
temp_wickets = wickets

for i in range(int(overs), 20):
    
    temp_balls_left = 120 - (i * 6)
    temp_wickets_left = 10 - temp_wickets
    
    if i == 0:
        temp_crr = 0
    else:
        temp_crr = temp_score / i
    
    temp_aggression = temp_crr * temp_wickets_left
    
    if temp_balls_left == 0:
        temp_pressure = 0
    else:
        temp_pressure = temp_wickets_left / temp_balls_left
    
    # Phase
    if i <= 6:
        temp_phase = "powerplay"
    elif i <= 15:
        temp_phase = "middle"
    else:
        temp_phase = "death"
    ...
    temp_progress = i / 20
    temp_runs_possible = (temp_balls_left/6) * temp_crr
    ...
    temp_df = pd.DataFrame({
        ...
        'wickets_left':[temp_wickets_left],
        'crr':[temp_crr],
        'last_five':[last_five],
        'aggression':[temp_aggression],
        'pressure':[temp_pressure],
        'phase':[temp_phase],
        'progress':[temp_progress],
        'runs_possible':[temp_runs_possible]
    })
    
    temp_result = pipe.predict(temp_df)
    
    overs_range.append(i)
    predicted_scores.append(temp_result[0])
    
    temp_score += temp_crr  # approximate score growth

# Plot graph
st.line_chart(pd.DataFrame({
    'Overs': overs_range,
    'Predicted Final Score': predicted_scores
}).set_index('Overs'))
